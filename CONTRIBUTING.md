...
The tests use a fake detector, so you do not need PyTorch to run them. To run real scans, also install the extra: `uv sync --extra zero-shot`.
...
3. Put heavy dependencies in an optional extra in `pyproject.toml`.

## Add a report format

Write a writer function in `src/tickterminator/reports.py`, then add it to `ReportFormat` and `_WRITERS`.

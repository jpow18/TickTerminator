...
Select the prompts and `min_score` with numbers. Label some photos (COCO format), then compare with `tickterminator evaluate`. See [examples/pine-processionary](examples/pine-processionary) for an example. Include the results in your pull request.
